...
aws s3 sync s3://${stack_s3_bucket}/data/${genomic_dataset_s3_object_key}/all/ /opt/local/hpds/all/
...
sudo docker run --name=hpds -v /opt/local/hpds:/opt/local/hpds -p 8080:8080  -p 9000:9000 -e JAVA_OPTS="-XX:+UseParallelGC -XX:SurvivorRatio=250 -Xms1g -Xmx110g -Xdebug -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:9000 -Dserver.port=8080 -Dspring.profiles.active=integrationB -DCACHE_SIZE=2500 -DSMALL_TASK_THREADS=1 -DLARGE_TASK_THREADS=1 -DSMALL_JOB_LIMIT=100 -DID_BATCH_SIZE=5000 -DALL_IDS_CONCEPT=NONE -DID_CUBE_NAME=NONE" -d $HPDS_IMAGE
