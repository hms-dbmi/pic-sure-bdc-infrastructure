...
for i in 1 2 3 4 5; do sudo /usr/local/bin/aws --region us-east-1 s3 cp s3://${stack_s3_bucket}${dataset_s3_object_key} /opt/local/hpds/hpds_data.tar.gz && break || sleep 45; done
cd /opt/local/hpds
