...
if [[ -z "$stack_s3_bucket" || -z "$environment_name" || -z "$target_stack" || -z "$env_private_dns_name" ]]; then
  echo "Error: --stack_s3_bucket, --environment_name, --target_stack, and --env_private_dns_name are required"
  exit 1
fi
