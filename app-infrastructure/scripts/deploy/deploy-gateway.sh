...
if [[ "$artifact_prefix" == "${target_stack}/banner-rollout/"* ]]; then
  [[ -n "$artifact_etag" ]] || { echo "ERROR: banner gateway deploy requires the verified artifact ETag." >&2; exit 2; }
  s3_get_exact "$stack_s3_bucket" "$artifact_prefix/pic-sure-gateway.tar.gz" "$artifact_etag" "/opt/picsure/pic-sure-gateway.tar.gz"
else
  s3_copy "s3://${stack_s3_bucket}/${artifact_prefix}/pic-sure-gateway.tar.gz" "/opt/picsure/pic-sure-gateway.tar.gz"
fi
