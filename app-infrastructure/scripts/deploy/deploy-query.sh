...
if [[ "$artifact_prefix" == "${target_stack}/banner-rollout/"* ]]; then
  [[ -n "$artifact_etag" ]] || { echo "ERROR: banner Query deploy requires the verified artifact ETag." >&2; exit 2; }
  s3_get_exact "$stack_s3_bucket" "$artifact_prefix/pic-sure-hpds-query-service.tar.gz" "$artifact_etag" "/opt/picsure/pic-sure-hpds-query-service.tar.gz"
else
