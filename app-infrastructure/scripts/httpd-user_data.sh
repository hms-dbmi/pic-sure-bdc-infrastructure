...
for i in 1 2 3 4 5; do echo "confirming wildfly resolvable" && wget $(grep -A30 preprod httpd-vhosts.conf | grep wildfly | grep api | cut -d "\"" -f 2 | sed 's/pic-sure-api-2.*//') && break || sleep 45; done
