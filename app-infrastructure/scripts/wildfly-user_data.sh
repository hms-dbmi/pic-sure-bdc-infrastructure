#!/usr/bin/env bash
sudo /usr/local/bin/aws --region us-east-1a s3 cp s3://avillach-datastage-pic-sure-jenkins-dev-builds-3/releases/jenkins_pipeline_build_${stack_githash}/pic-sure-wildfly.tar.gz .
