...
for i in 1 2 3 4 5; do echo "confirming hpds resolvable" && sudo curl -X POST --data "{}" -o curlOut.txt -H "Content-Type: application/json" -v $(grep hpds pic-sure-schema.sql | cut -d "'" -f2)/info && break || sleep 60; done
