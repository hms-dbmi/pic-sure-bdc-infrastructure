...
output "hpds-ec2-open-id" {
  value = var.include_open_hpds && length(aws_instance.open-hpds-ec2) > 0 ? aws_instance.open-hpds-ec2[0].id : ""
}

output "hpds-ec2-auth-id" {
  value = var.include_auth_hpds && length(aws_instance.auth-hpds-ec2) > 0 ? aws_instance.auth-hpds-ec2[0].id : ""
}