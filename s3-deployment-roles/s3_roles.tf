...
resource "aws_iam_instance_profile" "wildfly-deployment-s3-profile" {
  name = "wildfly-deployment-profile-${var.target-stack}${var.target-stack}-{var.stack_githash}"
  role = aws_iam_role.wildfly-deployment-s3-role.name
}

resource "aws_iam_role_policy" "wildfly-deployment-s3-policy" {
  name = "wildfly-deployment-policy-${var.target-stack}${var.target-stack}-${var.stack_githash}"
...
resource "aws_iam_role" "wildfly-deployment-s3-role" {
  name               = "wildfly-deployment-role-${var.target-stack}${var.target-stack}-${var.stack_githash}"
...
resource "aws_iam_instance_profile" "httpd-deployment-s3-profile" {
  name = "httpd-deployment-profile-${var.target-stack}${var.target-stack}-${var.stack_githash}"
  role = aws_iam_role.httpd-deployment-s3-role.name
}

resource "aws_iam_role_policy" "httpd-deployment-s3-policy" {
  name = "httpd-deployment-policy-${var.target-stack}${var.target-stack}-${var.stack_githash}"
...
resource "aws_iam_role" "httpd-deployment-s3-role" {
  name               = "httpd-deployment-role-${var.target-stack}${var.target-stack}-${var.stack_githash}"
  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_instance_profile" "hpds-deployment-s3-profile" {
  name = "hpds-deployment-profile-${var.target-stack}${var.target-stack}-${var.stack_githash}"
  role = aws_iam_role.hpds-deployment-s3-role.name
}

resource "aws_iam_role_policy" "hpds-deployment-s3-policy" {
  name = "hpds-deployment-policy-${var.target-stack}${var.target-stack}-${var.stack_githash}"
...
resource "aws_iam_role" "hpds-deployment-s3-role" {
  name               = "hpds-deployment-s3-role-${var.target-stack}${var.target-stack}-${var.stack_githash}"
  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
