...
resource "aws_s3_bucket_object" "session-logs-folder" {
  ...
  key          = join("", ["genomic-etl/deployment_state_metadata/",var.study_id, "/", var.chrom_number, "/var.session_logs/"])
  content_type = "application/x-directory"
}

resource "aws_s3_bucket_object" "genomic-etl-tfstate-baseline" {
  bucket  = var.output_s3_bucket
  key     = join("", ["genomic-etl/deployment_state_metadata/",var.study_id, "/", var.chrom_number, "/terraform.tfstate"])
...
resource "aws_s3_bucket_object" "genomic-etl-variables-baseline" {
  bucket  = var.output_s3_bucket
  key     = join("", ["genomic-etl/deployment_state_metadata/",var.study_id, "/", var.chrom_number,"/genomic_etl_variables.tf"])
