resource "aws_security_group" "inbound-from-public-internet" {
  ...
  tags = {
    ...
    Name        = "FISMA Terraform Playground - inbound-from-public-internet Security Group"
  }
}

resource "aws_security_group" "outbound-to-app" {
  name = "allow_outbound_from_edge_to_wildfly_port_in_app_vpc"
  description = "Allow outbound traffic to app-private-subnets on port 8080 until we have TLS in place for app server"
  vpc_id = aws_vpc.edge-vpc.id
  ...
  tags = {
    Owner       = "Avillach_Lab"
    Environment = "development"
    Name        = "FISMA Terraform Playground - outbound-to-app Security Group"
  }
}
